...
def train(hyp, opt, device, tb_writer=None):
  ...
  # Add this right after epoch_losses.append(...) line
    plt.figure(figsize=(10, 5))
    plt.plot(range(start_epoch, epochs), epoch_losses, label='Training Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Epoch vs Loss')
    plt.legend()
    loss_plot_path = save_dir / 'loss_vs_epoch.png'  # Set the path for saving the loss plot
    plt.savefig(loss_plot_path)  # Save the plot
    plt.close()  # Close the plot to avoid display

# Log the loss plot to W&B if applicable
    if wandb_logger.wandb:
        wandb_logger.log({"Loss Plot": wandb_logger.wandb.Image(str(loss_plot_path), caption='Loss vs Epoch')})
    ...
    if rank in [-1, 0]:
        if plots:
            ...
            if wandb_logger.wandb:
                files = ['results.png', 'confusion_matrix.png', 'loss_vs_epoch.png', *[f'{x}_curve.png' for x in ('F1', 'PR', 'P', 'R')]]
